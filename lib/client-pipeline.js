  ...
  constructor (opts, handler) {
    ...
    this.req = new Readable({
      ...
      destroy: (err, callback) => {
        ...
        console.error('req.destroy', new Error().stack)

        if (err) {
          console.error('req.destroy 1')
          util.destroy(ret, err)
        } else if (!req._readableState.endEmitted) {
          // This can happen if the server doesn't care
          // about the entire request body.
          console.error('req.destroy 2')
          ret.end()
          console.error('req.destroy 3')
        }
    ...
    this.ret = new Duplex({
      ...
      console.error('ret."end"')
    }).on('finish', () => {
      console.error('ret."finish"')
    }).on('close', () => {
      console.error('ret."close"')
    })
