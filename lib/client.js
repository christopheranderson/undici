...
function write (client, request) {
  ...
  if (contentLength !== null) {
    position += header.write('content-length: ', position)
    position += header.write(String(contentLength), position)
    position += header.write('\r\n', position)
  }
  ...
  if (!body) {
    ...
    socket.write(header.slice(0, position))
