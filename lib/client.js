...
const kSocketWriting = Symbol('socket writing')
const kSocketBuffer = Symbol('socket buffer')
const kSocketOnWrite = Symbol('socket on write')

function fastSocket (socket) {
  socket[kSocketBuffer] = []
  socket[kSocketWriting] = false
  socket[kSocketOnWrite] = function (err) {
    this[kSocketWriting] = false
    if (err) {
      this.destroy(err)
    } else if (this[kSocketBuffer].length) {
      this.write()
    } else {
      this.emit('drain')
    }
  ...
  socket.write = function (chunk) {
    if (chunk) {
      this[kSocketBuffer].push({ chunk })
    }

    if (this[kSocketWriting]) {
      return
    }

    this[kSocketWriting] = true

    const buffer = this[kSocketBuffer]
    buffer.allBuffers = true

    this[kSocketBuffer] = []
    this._writev(buffer, this[kSocketOnWrite])
  }
}
...
function write (client, request) {
  ...
  if (contentLength !== null) {
    socket.write(Buffer.from(`content-length: ${contentLength}\r\n`, 'ascii'))
  }
  ...
  if (!body) {
    ...
    const onData = function (chunk) {
      ...
      if (bytesWritten === 0) {
        socket.write(Buffer.from(contentLength === null ? 'transfer-encoding: chunked\r\n' : '\r\n', 'ascii'))
        client[kReset] = !expectsPayload
      }
